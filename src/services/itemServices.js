...
const getNews = async () => {
  ...
  items.forEach((item) => {
    ...
    const diferenciaDias =
      (fecha_actual - fecha_creacion) / (1000 * 60 * 60 * 24);
  ...
  const itemSchema = {
    ...
    image_front:
      files && files.length > 0 && files[0].filename
        ? "/" + files[0].filename
        : existingItem.image_front,
    image_back:
      files && files.length > 1 && files[1].filename
        ? "/" + files[1].filename
        : existingItem.image_back,
    licence_id: item.licence,
    category_id: item.category,
  };
...
const deleteItem = async (id) => {
  ...
  const files = [path + item.image_front, path + item.image_back];
